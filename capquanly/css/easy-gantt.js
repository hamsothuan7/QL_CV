$.fn.gantt = function (options) {
    ...
    $.each(tasks, function(index, task) {
        ...
        if(tooltipShow){
            $('#tbody_'+unic+' > tr > .td-tasks').on('mouseover', function(e){
                var infoHtml = `<b>${$(this).attr('task_name')}</b><br>
                    <span>${$(this).attr('start')} - ${$(this).attr('end')}</span><br>
                    <span>${$(this).attr('task_days')} ngày</span>`;
                var desc = $(this).attr('tooltip_desc');
                if (desc) infoHtml += `<hr><span>${desc}</span>`;
                $('#ganttTaskInfoFloat').html(infoHtml).show();
            }).on('mousemove', function(e){
                var $info = $('#ganttTaskInfoFloat');
                var tooltipWidth = $info.outerWidth();
                var tooltipHeight = $info.outerHeight();
                var pageWidth = $(window).width();
                var pageHeight = $(window).height();
                var left = e.pageX + 20;
                var top = e.pageY + 10;
                if (left + tooltipWidth > pageWidth) left = e.pageX - tooltipWidth - 20;
                if (left < 0) left = 0;
                if (top + tooltipHeight > pageHeight) top = e.pageY - tooltipHeight - 10;
                if (top < 0) top = 0;
                $info.css({top: top, left: left});
            }).on('mouseout', function(){
                $('#ganttTaskInfoFloat').hide().html('');
            });
        }
    });
