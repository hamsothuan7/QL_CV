    ...
    if (!$conn) {
        ...
        if ($ngaybd > $ngaykt) {
            ...
            if (mysqli_num_rows($check_result) == 0) {
                ...
                $pb_ma = $row['PB_MA'];
                $query = "UPDATE `danhsachcongviec` 
                          SET `DSCV_TEN`='$tencv', `DSCV_NGAYBATDAU`='$ngaybd', `DSCV_NGAYKETTHUC`='$ngaykt', `DSCV_TRANGTHAI`='$tinhtrang', `TV_MA`='$tv_ma', `DA_MA`='$duan_ma', `PB_MA`='$pb_ma' 
                          WHERE `DSCV_MA`='$id'";
                ...
                if ($result == true) {
                    ...
                    echo "Update data error: " . mysqli_error($conn);
                }
            }
        }
        mysqli_close($conn);
...
$id = $_GET['id'];
$task_query = "SELECT * FROM `danhsachcongviec` WHERE `DSCV_MA`='$id'";
    ...
    <div id="content">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h2 class="m-0">Chỉnh Sửa Công Việc</h2>
                    </div>
                </div>
            ...
            <div class="container-fluid">
                <div class="col-lg-12">
                    <div class="card card-outline card-primary" style="border-top: 3px solid rgb(48, 162, 48); border-radius: 5px;">
                        <div class="card-body">
                            <form method="post">
                                <input type="hidden" name="taskId" value="<?php echo $task['DSCV_MA']; ?>">
                                <div class="row">
                                    <div class="col-md-6 border-right">
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            <label>Tên Công Việc :</label>
                                            <input type="text" class="form-control form-control-sm" required name="txtName" value="<?php echo $task['DSCV_TEN']; ?>">
                                        </div>
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            <label>Ngày Bắt Đầu</label>
                                            <input type="date" class="form-control form-control-sm" autocomplete="off" required name="txtNgayBD" value="<?php echo $task['DSCV_NGAYBATDAU']; ?>">
                                        </div>
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            <label>Ngày Kết Thúc</label>
                                            <input type="date" class="form-control form-control-sm" autocomplete="off" required name="txtNgayKT" value="<?php echo $task['DSCV_NGAYKETTHUC']; ?>">
                                        </div>
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            <label>Tình Trạng :</label>
                                            <div>
                                                <select id="tinhtrang" name="tinhtrang" class="custom-select mb-3" style="margin: 0px;">
                                                <option value="1" <?php if ($task['DSCV_TRANGTHAI'] == 1) echo 'selected'; ?>>Đang Tiến Hành</option>
                                                    <option value="2" <?php if ($task['DSCV_TRANGTHAI'] == 2) echo 'selected'; ?>>Hoàn Thành</option>
                                                    <option value="3" <?php if ($task['DSCV_TRANGTHAI'] == 3) echo 'selected'; ?>>Hoãn lại</option>
                                                    <option value="4" <?php if ($task['DSCV_TRANGTHAI'] == 4) echo 'selected'; ?>>Hủy</option>
                                                    <option value="5" <?php if ($task['DSCV_TRANGTHAI'] == 5) echo 'selected'; ?>>Bắt Đầu</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            ...
                                            <div>
                                                <select id="selectDuan" name="selectDuan" class="custom-select mb-3" style="margin: 0px;">
                                                    ...
                                                    foreach ($duan_arr as $duan) {
                                                        echo '<option value="' . $duan['DA_MA'] . '" ' . ($task['DA_MA'] == $duan['DA_MA'] ? 'selected' : '') . '>' . $duan['DA_TEN'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                <select id="selectPhongBan" name="selectPhongBan" class="custom-select mb-3" style="margin: 0px;" onchange="fetchThanhVien()">
                                                    ...
                                                    foreach ($phongban_arr as $phongban) {
                                                        echo '<option value="' . $phongban['PB_MA'] . '" ' . ($task['PB_MA'] == $phongban['PB_MA'] ? 'selected' : '') . '>' . $phongban['PB_TEN'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-group" style="margin-bottom: 0rem;">
                                            ...
                                            <div>
                                                <select id="selectThanhVien" name="selectThanhVien" class="custom-select mb-3" style="margin: 0px;">
                                                    ...
                                                    <?php
                                                    $thanhvien_query = "SELECT TV_MA, TV_TEN FROM thanhvien WHERE PB_MA = '".$task['PB_MA']."'";
                                                    $thanhvien_result = mysqli_query($conn, $thanhvien_query);
                                                    while ($thanhvien = mysqli_fetch_assoc($thanhvien_result)) {
                                                        echo '<option value="' . $thanhvien['TV_MA'] . '" ' . ($task['TV_MA'] == $thanhvien['TV_MA'] ? 'selected' : '') . '>' . $thanhvien['TV_TEN'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
